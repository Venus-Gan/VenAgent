...
| CFG-01 | 默认→共享→local→env→CLI 配置分层 | 未完成：当前候选文件择一 | 不可变配置与明确深度合并规则 | P1-01～P1-03 | 优先级、mapping/list 和未知字段测试通过 |
| CFG-02 | secret 不进入版本控制 | 未完成：默认配置仍有密码；另有外部仓库疑似泄漏 | local/env/secret reference + 自动脱敏 | P0-01～P0-04、P1-03 | 活动凭据已轮换；tracked 文件无真实 secret |
| CFG-03 | 基础设施按能力降级 | 已完成基础能力 | capability health 明确 durable/non-durable 可用性 | P1-08、P1-10 | 单个设施失败不阻塞无关功能，状态可诊断 |
| CFG-04 | 分阶段启动且失败可诊断 | 未完成：副作用仍在构造器 | Bootstrapper + FastAPI lifespan | P1-04～P1-10 | 初始化/关闭顺序可测，资源无泄漏 |
...
| NFR-01 | HTTP API 与前端交互不变 | 已完成重要基线 | Handler/RuntimeEvent 兼容边界 | P0-07、P3-04、P3-14、P6-03、P7-04 | characterization、OpenAPI schema diff 和关键 E2E 通过 |
...
| 依赖 | 用途 | 引入时点 | 当前状态 |
|---|---|---|---|
| 账号所有者操作 | 撤销/轮换 AGI-saber 疑似凭据、检查调用和计费 | Phase 0 前置 | 阻塞 P0-01，不能由代码代理代办 |
...
| D-11 | 性能回归阈值 | **Phase 0 测量后，以 p95 TTFT/完成时间不劣化 15% 为初始门禁** | 用户指定其他阈值 | 已确认（2026-07-18） | 决定 Phase 6 发布门禁 |
| D-12 | 依赖引入授权 | **允许在 P0-10 隔离环境验证，版本确认后再改 requirements** | 只做文档研究 / 立即加入生产 requirements | 已确认（2026-07-18） | 决定能否验证真实兼容性 |
| D-13 | 已发现疑似凭据 | **立即由账号所有者撤销、轮换并审计使用记录** | 仅确认无效后记录豁免 | 已确认（2026-07-18） | 这是安全阻断项，不应等待重构实施 |
...
- [x] 需用户决策事项集中列出；
- [x] 用户确认第六节全部决策；
- [x] Phase 0 执行计划获批准，但尚未启动。

本次提交冻结已确认的规划基线，不代表 Phase 0 已经执行，也不代表可以跳过各任务自身的安全门禁、依赖版本确认或外部账号操作。
