...
| Phase | 名称 | 核心交付 | 状态 | 依赖 |
|---|---|---|---|---|
| 0 | 安全、冻结与基线 | 凭据处置、行为/测试/性能基线、依赖 spike | READY | 无 |
| 1 | 配置与 Bootstrap | `src/apps` 骨架、完整配置分层、lifespan、显式 container | 待开始 | Phase 0 |
...
1. 立即撤销并轮换 AGI-saber 工作树中发现的两项疑似真实 API 凭据。
2. 扫描 AGI-saber 与 VenAgent 全 refs 和历史；如命中，先确认所有凭据已轮换，再记录风险，并在仓库所有者授权后执行历史清理；若暂未授权，必须形成明确的安全豁免/阻断记录，不能宣称历史零命中。
3. 移除两仓库已跟踪配置/Compose 中可直接使用的默认凭据；真实值移至 ignored local/env/secret。
...
7. 为 HTTP/SSE/OpenAPI/chat/RAG/tool/document/取消/恢复建立 characterization tests。
8. 记录关键性能基线：chat 首 token、RAG 完成、workflow 完成、启动时间和 graceful degradation。
...
- [ ] 疑似真实凭据已撤销/轮换，活动凭据不再存在于工作树或跟踪配置。
- [ ] 所有 refs/历史已扫描；历史命中已在获得所有者授权后清理，或存在明确、经批准且标明剩余风险的阻断/豁免记录。
- [ ] 跟踪文件不含真实 API Key/Token/可部署默认密码。
- [ ] local secret 文件被 ignore 且未 tracked。
- [ ] `cd final && python -m pytest tests` 通过。
- [ ] `python -m compileall -q final` 通过。
- [ ] characterization tests 覆盖现有外部契约。
...
- [ ] HTTP/SSE characterization tests 零变化。
...
用官方 LangGraph 和官方 checkpointer 接管澄清、聊天和 RAG，不触碰副作用工作流。
...
11. 提供 per-profile runtime feature flag。
12. 对只读路径执行 legacy/new shadow comparison。
...
8. 发布前执行完整测试、语法、覆盖率（若可用）、FastAPI、安全、性能和 E2E 门禁。
