...
class UnifiedAgent:
    ...
    def __init__(self, cfg: APIConfig, inf: Infrastructure):
        ...
        self.governance = RuntimeGovernance(self)
    ...
    def process_with_options(self, query: str, opts: ChatOptions) -> Response:
        token, unregister = self._governance().register()
        try:
    ...
    def _governance(self):
        ...
        if governance is None:
            governance = RuntimeGovernance(self)
    ...
    def infra_status(self) -> Dict[str, str]:
        return self._governance().infra_status()
    ...
    def process_stream_with_options(
        self,
        query: str,
        opts: ChatOptions,
        token,
        ...
        """流式编排入口：prepare 一次，所有模式共用同一 SSE 通道，最后统一 finalize。"""
        pr = self._prepare(query, opts)
        resp = Response(query=query, mode=pr["mode"])
        resp.extracted_info = pr["extracted"]

        self._emit_stream_event(on_event, "route", {"mode": resp.mode})
        if resp.extracted_info:
            self._emit_stream_event(on_event, "memory", {"extracted_info": resp.extracted_info})
        ...
        if pr["mode"] == "chat":
            resp.answer = self._chat_response_stream(
                pr["mem_prefix"],
                pr["hist_msgs"],
                token,
                on_token=on_token,
    ...
    def _emit_stream_event(
        ...
        try:
            ...
            logger.warning("stream event 回调异常 [%s]: %s", event, e)
    ...
    def _intent_policy(self) -> IntentPolicy:
        policy = getattr(self, "intent_policy", None)
        if policy is None:
            policy = IntentPolicy()
            self.intent_policy = policy
        return policy

    def _build_intent_signal(self, query: str, opts: ChatOptions) -> IntentSignal:
        tool_names: tuple[str, ...] = ()
        tool_executor = getattr(self, "tool_executor", None)
        if tool_executor is not None and hasattr(tool_executor, "snapshot"):
            try:
                tool_names = tuple(tool_executor.snapshot().keys())
            except Exception:
                tool_names = ()

        agent_names: tuple[str, ...] = ()
        subagents = getattr(self, "subagents", None)
        if subagents is not None and hasattr(subagents, "names"):
            try:
                agent_names = tuple(subagents.names())
            except Exception:
                agent_names = ()
        ...
        return IntentSignal(
            ...
            selected_tools=tuple(opts.selected_tools or ()),
    ...
    @staticmethod
    def _mode_from_intent(decision) -> str:
        profile = decision.execution_profile
        if profile is ExecutionProfile.KNOWLEDGE_ANSWER:
            return "rag"
        if profile is ExecutionProfile.SINGLE_TOOL:
            return "tool"
        if profile in {ExecutionProfile.WORKFLOW_TASK, ExecutionProfile.DOCUMENT_FLOW}:
            return "react"
        return "chat"
    ...
    def _planner_snapshot(self):
        task = self._governance().current_task()
        if not task:
            return None
    ...
    def save_snapshot(self, task: dict) -> None:
        self._governance().save_snapshot(task)

    def snapshot_list(self) -> List[dict]:
        """返回当前任务的内存快照列表（对应 main snapshotList）。"""
        return self._governance().snapshot_list()
    ...
    def _run_rag_query(
        ...
        if self.rag is None:
            return "RAG 不可用", []
        history = self._recent_history_for_rag()
        if on_event is not None:
            self._emit_stream_event(on_event, "rag_progress", {"stage": "retrieve", "query": query})
        if hasattr(self.rag, "query_with_history_stream"):
            return self.rag.query_with_history_stream(query, history, ctx=token, on_token=on_token, on_event=on_event)
    ...
    def _chat_system_prompt(self, mem_prefix: str) -> str:
    ...
    def _chat_response(self, mem_prefix: str, hist_msgs: List[Message]) -> str:
        return self.llm.chat(hist_msgs, system_prompt=self._chat_system_prompt(mem_prefix))

    def _chat_response_stream(
        self,
        mem_prefix: str,
        ...
        return self.llm.chat_stream_context(
            token,
            system_prompt=self._chat_system_prompt(mem_prefix),
            messages=hist_msgs,
            on_token=on_token,
        )
    ...
    def _run_react_with_tools(
        ...
        try:
            ...
            if on_event is not None:
                self._emit_stream_event(on_event, "react_progress", {"stage": "final_answer", "count": len(steps)})
            final_answer = self._generate_final_answer_stream(
                query,
                steps,
                mem_prefix,
                token,
    ...
    def _generate_final_answer(self, query: str, steps: List[ReActStep], mem_prefix: str) -> str:
        ...
        messages = [Message(role="user", content=self._final_answer_user_msg(query, steps_str, mem_prefix))]
        return self.llm.chat(messages, system_prompt=self._final_answer_system_prompt())
    ...
    def _generate_final_answer_stream(
        ...
        return self.llm.chat_stream_context(
            token,
            system_prompt=self._final_answer_system_prompt(),
            messages=messages,
            on_token=on_token,
        )

    def _final_answer_system_prompt(self) -> str:
        return "你是一个总结助手，能够基于推理过程给出简洁的最终答案。"

    def _final_answer_user_msg(self, query: str, steps_str: str, mem_prefix: str) -> str:
        return f"""基于以下推理过程，给出最终答案。
