...
class GraphRuntime:
    ...
    def execute(self, token) -> GraphResult:
        ...
        for idx, level in enumerate(levels):
            if _is_cancelled(token):
                self._emit_event({"type": "level_cancelled", "level": idx})
                return self._build_interrupted(f"在第 {idx} 层执行前被中断")

            pending_level = [node_id for node_id in level if self.graph.nodes[node_id].status == NodeStatus.PENDING]
            self._emit_event({"type": "level_start", "level": idx, "nodes": list(pending_level)})
            if not pending_level:
                self._save_snapshot()
                self._emit_event({"type": "level_done", "level": idx})
                continue
            groups = self._group_by_race(pending_level)
            threads = []
            ...
            if _is_cancelled(token):
                self._emit_event({"type": "level_cancelled", "level": idx, "after": True})
    ...
    def _execute_group(self, token, _group_name: str, node_ids: List[str]) -> None:
        ...
        for node_id in node_ids:
            if self.graph.nodes[node_id].status != NodeStatus.PENDING:
                continue
    ...
    def _race_group(self, token, group_name: str, node_ids: List[str]) -> None:
        ...
        def runner(node_id: str) -> None:
            if done.is_set() or _is_cancelled(token) or self.graph.nodes[node_id].status != NodeStatus.PENDING:
                self.graph.set_node_status(node_id, NodeStatus.CANCELLED)
                return
            self._emit_event({"type": "node_start", "node": node_id, "group": group_name})
            ...
            with results_lock:
                ...
                if error is None and not done.is_set():
                    ...
                    self._emit_event({"type": "race_winner", "group": group_name, "node": node_id})
                    _cancel_token(token)
