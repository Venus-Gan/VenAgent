...
class Engine:
    ...
    def _compose_answer(self, question: str, fused: List[dict]) -> Tuple[str, List[dict]]:
        ...
        if self._generate_fn:
            ...
            user_msg = self._answer_user_msg(question, context)
    ...
    def _compose_answer_stream(
        ...
        if self._generate_fn and self._llm is not None:
            messages = [Message(role="user", content=self._answer_user_msg(question, context))]
            stream_fn = getattr(self._llm, "chat_stream_context", None)
            if callable(stream_fn):
                ...
                return answer, fused
            chat_fn = getattr(self._llm, "chat", None)
            if callable(chat_fn):
                answer = chat_fn(messages, system_prompt=self._answer_system_prompt())
            else:
                answer = self._generate_fn(self._answer_system_prompt(), self._answer_user_msg(question, context))
            if on_token:
                for ch in answer:
                    on_token(ch)
            return answer, fused
    ...
    def _answer_user_msg(self, question: str, context: str) -> str:
        return f"上下文：\n{context}\n\n问题：{question}"

    def query_with_history_stream(
        self,
        question: str,
        ...
        if on_event is not None:
            try:
                on_event("rag_progress", {"stage": "rewrite", "question": question})
            except Exception:
                pass
        if not self.loaded:
            return "知识库为空，请先上传文档。", []
        ...
        if on_event is not None:
            try:
                on_event("rag_progress", {"stage": "search", "queries": queries})
            except Exception:
                pass
        ...
        if self._hybrid is not None:
            ...
            if on_event is not None:
                try:
                    on_event("rag_progress", {"stage": "fused", "count": len(fused)})
                except Exception:
                    pass
            ask_query = queries[0] if queries else question
            return self._compose_answer_stream(ask_query, fused, ctx, on_token=on_token)

        return self._compose_answer_stream(question, [], ctx, on_token=on_token)
