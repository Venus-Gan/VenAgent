...
from .mcp_catalog import tools_to_catalog_entries
...
def search_web_factory(cfg=None, llm=None) -> Callable[[Dict[str, Any]], str]:
    ...
    默认路径通过标准化后端适配层工作，不再依赖 provider key。
    llm 可用时走知识库式回答，否则回退到 mock 搜索。
    cfg 参数保留兼容性，但默认搜索路径不读取 provider 分支。
    """
...
def default_tools(cfg=None, llm=None, sandbox=None) -> List[Tool]:
    ...
    tools: List[Tool] = [
        ...
        Tool(
            name="search_web",
            description="执行网络搜索（标准后端 → LLM 知识库 → mock 降级）",
            params=[{"name": "query", "type": "string", "description": "搜索关键词"}],
            func=search_func,
        ),
    ]
...
class ToolExecutor:
    ...
    def get_tool_descriptions(self) -> List[Dict[str, Any]]:
        ...
        return tools_to_catalog_entries(tools)
