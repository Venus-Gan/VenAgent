...
def test_graph_runtime_executes_dependency_levels_and_records_context():
    ...
    assert any(evt["type"] == "level_start" for evt in events)
    assert any(evt["type"] == "level_done" for evt in events)
...
def test_graph_runtime_race_group_first_success_wins():
    ...
    assert any(evt["type"] == "race_winner" for evt in events)
...
def test_graph_runtime_skips_nodes_already_marked_done_for_resume():
    calls = []
    graph = TaskGraph([
        Node(id="n1", type=NodeType.TOOL, tool_name="a"),
        Node(id="n2", type=NodeType.TOOL, tool_name="b", depends_on=["n1"]),
    ])
    graph.set_node_result("n1", "A")
