...
def test_engine_query_with_history_stream_emits_tokens_and_progress():
    inf = _FakeInfra()
    engine = Engine(_FakeCfg(), inf, _FakeLLM())
    engine.loaded = True
    engine.set_rewriter(LLMRewriter(lambda _system, _user: json.dumps({"queries": ["main", "alt"]}), 2))
    engine.set_generate_fn(lambda _system, _user: "stream-answer")
    tokens = []
    events = []

    answer, results = engine.query_with_history_stream(
        "原问题",
        [HistoryMessage(role="user", content="历史")],
        ctx=SimpleNamespace(is_cancelled=lambda: False),
        on_token=tokens.append,
        on_event=lambda event, payload: events.append((event, payload)),
    )

    assert answer == "stream-answer"
    assert tokens == list("stream-answer")
    assert [r["content"] for r in results] == ["parent A", "parent B"]
    assert [p["stage"] for e, p in events if e == "rag_progress"] == ["rewrite", "search", "fused"]
